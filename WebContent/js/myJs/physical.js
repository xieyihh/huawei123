...
$(function() {
	...
	$(document).on("click", "[id^='relative']", function() {// 修改成这样的写法
		...
		// "relatives.jsp?physicalplan="+plan+"&usernumber="+number+"&username="+name;
	...
	// data: {"id": listNum,"physicalreservedate":$(this).val()},
...
function BindData() {
	...
	$.ajax({
		...
		data : {
			...
			"physicalreservedate" : physicalreservedate,
		...
		success : function(msg) {// msg为返回的数据，在这里做数据绑定
			...
			$.each(msg.context.physicallist, function(i, item) {
				...
				var bodyContent = '<tr>' + '<td  id="number' + item.id + '" >'
						+ item.usernumber + '</td>' + '<td  id="name' + item.id
						+ '">' + item.username + '</td>' + '<td  >'
						+ item.physicalposition + '</td>' + '<td  >'
						+ item.physicalreservedate + '</td>' + // id="changeDate'+item.id+'"
						'<td >' + item.physicaldate + '</td>' + '<td >'
						+ item.physicalstate + '</td>' + '<td  id="plan'
						+ item.id + '" >' + item.physicalplan + ' </td>';
				// " </td><td width=\"10%\">" + item.physicalnumber+
				if (item.hasRelatives == "无") {
					bodyContent += '<td >' + item.hasRelatives + '</td>';
				} else {
					bodyContent += '<td ><a id="relative' + item.id + '" >'
							+ "查看家属详情" + ' </a></td>';
				}
				bodyContent +=
				// '<td width="10%"><a id="delete'+item.id+'" > <img
				// src="img/delete.gif" border="0" title="删除"></a> </td>'+
				'</tr>';
				$("#tb_body").append(bodyContent);
				/*
				 * <td width="10%"><a id="edit'+list+'" >' + "编辑" + ' </a></td>
				 * $("#tb_body").append("<tr><td>" + 12+ "</td><td>" + 12 + "
				 * </td><td>" +12+ " </td><td>" + 2 + " </td><td>" + 2 + "
				 * </td><td>" + 12 + "</td><td>" + 12+ " </td></tr>");
				 */
			...
			$.each(msg.context.physical_plan, function(i, item) {
				...
				if (count == sel.options.length) {
					$("select#physicalPlan").append(
							' <option value=' + (i + 1) + '>' + item
									+ '</option>');
				}
			...
			$.each(msg.context.physical_position, function(i, item) {
				...
				if (count == sel.options.length) {
					$("select#physicalposition").append(
							' <option value=' + (i + 1) + '>' + item
									+ '</option>');
				}
