...
/**
 * 产生缩略图的工具
 * @author 雷晓冰	2014-07-22
 */
public class ThumbnailUtil {
	/**
	 * 获取缩略图
	 * @param inputFile	原图
	 * @param thumbnailDirectory	存放缩略图的子目录
	 * @param thumbnailWidth			缩略图的宽度
	 * @return
	 */
	public static boolean generateThumbnail(File inputFile, File outputFile, int thumbnailWidth) {
		try {
			/*
			...
			BufferedImage bis = ImageIO.read(inputFile); // 读取图片
			int w = bis.getWidth();
			int h = bis.getHeight();
			//不需要生成缩略图
			if(w <= thumbnailWidth) {
			...
			int nh = (thumbnailWidth * h) / w;
			double sx = (double) thumbnailWidth / w;
			...
			BufferedImage bid = new BufferedImage(thumbnailWidth, nh,
			 ...
			 */
			ato.filter(bis, bid);
			ImageIO.write(bid, "jpg", outputFile);
		} catch (Exception e) {
			System.out.println("Parent : " + inputFile.getParent() + "\tName : " + inputFile.getName());
			return false;
		}
		return true;
	}
